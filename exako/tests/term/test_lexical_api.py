...
from exako.apps.term.api.schema import TermLexicalView
...
def test_create_term_lexical_term_ref_invalid_language_reference(
    ...
    payload = generate_payload(
        TermLexicalFactory,
        term=TermFactory(language=Language.DEUTSCH),
        value=None,
        term_value_ref=term,
    )
    ...
    assert response.status_code == 422
    assert (
        'term_value_ref language have to be the same as term lexical reference.'
