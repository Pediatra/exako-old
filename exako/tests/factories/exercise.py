...
class ListenTermLexicalTermRefFactory(_ExerciseBase):
    ...
    def _adjust_kwargs(cls, **kwargs):
        ...
        if 'term_pronunciation' not in kwargs:
            term_value_ref = kwargs['term_lexical'].term_value_ref
...
class TermMChoiceFactory(_ExerciseBase):
    ...
    @classmethod
    def _create_example_highlight(cls, **kwargs):
...
class TermMChoiceLexicalFactory(TermMChoiceFactory):
    term = None
    term_lexical = factory.SubFactory(TermLexicalFactory)
...
class TermMChoiceLexicalTermRefFactory(TermMChoiceFactory):
    term = None

    @staticmethod
    def _make_distractors():
        distractors = [
            TermLexicalFactory(value=None, term_value_ref=TermFactory())
            for _ in range(6)
        ]
        return {'term_lexical': [distractor.id for distractor in distractors]}
    ...
    def _create_example_highlight(cls, **kwargs):
        term_lexical = kwargs.get('term_lexical')
        if isinstance(term_lexical, int):
            term_lexical = TermLexical.objects.get(id=term_lexical)
        TermExampleLink.objects.get_or_create(
            highlight=[[1, 5], [7, 9]],
            term_example=kwargs.get('term_example'),
            term=term_lexical.term_value_ref,
        )
    ...
    def _adjust_kwargs(cls, **kwargs):
        term_value_ref = TermFactory()
        term_lexical = TermLexicalFactory(value=None, term_value_ref=term_value_ref)
        kwargs['term_lexical'] = kwargs.pop('term_lexical', None) or term_lexical
        return kwargs
